  ...
  <div>
    ...
    <div class="tab-bar">
      <van-tabbar route>
        ...
        <van-button block type="info" class="" @click="topBarClickLeft">{{
...
import { withdrawallogin } from "@/api/wallet";

import {
  getshoppicwalletincentivemodelandwithdrawalsettings,
  shoppicwithdrawal,
  getshoppicwithdrawalrecommendedamount,
} from "@/api/picWallet";
...
export default {
  ...
  data() {
    return {
      topBarTitle: this.$t("pmt.picwallet.topBarTitle"),
  ...
  methods: {
    ...
    onClickFinalConfirm() {
      ...
      this.$toast.loading({ duration: 0, forbidClick: true });
      shoppicwithdrawal(this.submitData)
        .then((res) => {
          const { success, data } = res;
          if (success) {
            this.handleHideLoading();
    ...
    onClickHistory() {
      this.$router.push({ name: "picWalletHostory" });
    },
    onClickDetail() {
      // console.log('onClickDetail')

      this.$router.push({ name: "picWalletDetail" });
    },
  },
};
</script>
