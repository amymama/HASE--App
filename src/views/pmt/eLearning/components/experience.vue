      ...
      <van-pull-refresh v-model="isLoading" @refresh="onRefresh">
        ...
        >
          ...
          >
            <van-card
              :centered="true"
              :title="item.new_sharing_name"
              :desc="getValidate(item.createdon)"
              :thumb="itemIcon"
              @click="onClick_Experience(item.Id)"
            >
              <template #desc>
...
import { postSharingPage, getSharingClicked } from "@/api/eLearning";
import { postUserPermission } from "@/api/user";
import { getPmtSelectData } from "@/api/common";
import ChangeLang from "@/components/ChangeLang";

import listItemIcon from "@/assets/images/pmt/e-learning/Sharing_home_icon.png";
...
export default {
  ...
  methods: {
    ...
    onLoad() {
      ...
      postSharingPage(data)
        .then((res) => {
          const { success, data } = res;
          if (success) {
            if (success && data.Items && data.Items.length > 0) {
              this.experienceList = [...this.experienceList, ...data.Items];
