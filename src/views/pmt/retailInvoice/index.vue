<template>
  <div class="retailBox">
    <TopBar
      :topBarTitle="this.$t('pmt.retail.titleInvoice')"
        ...
        <div class="van-cell" v-for="(item, i) in addModelList" :key="i">
          <van-swipe-cell>
            <van-field
              ...
              readonly
              :label="$t('pmt.model')"
              @click="(selectModel = i), (showModelPicker = true)"
              input-align="right"
              label-width="90px"
              :rules="[
                {
...
import {
  ...
  getusershopinfo,
  getentityconditions,
  getmodelbrandbymodel,
} from "@/api/retail";
import TopBar from "@/components/TopBar";
import {
  getretailinvoicemodel,
  saveretailinvoicemodel,
  submitretailinvoicemodel,
  getdropdownoptions,
} from "../../../api/retail";
Vue.use(Overlay);
export default {
  ...
  data() {
    return {
      ...
      formData: {
        ...
        new_consumer_address: "", // TODO 需确认F
      },
      createOn: "",
      pageType: 1,
      editable: true,
  ...
  created() {
    ...
    if(this.type == 3)
    {
      ...
      console.log(this.isSaveSuccess)
    }
  },
  mounted() {
  ...
  methods: {
    ...
    qrCode(data) {
      ...
      retailmobileScanbarcode({
        ...
        if (res.success === true) {
          ...
          item.new_modelid = res.data.new_model_id;
        } else {
    ...
    onSave() {
      ...
      const queryParam = {
        ...
        new_shop_name: this.formData.new_shop_name,
      ...
      console.log(params);
      this.$toast.loading({ duration: 0, forbidClick: true });
      saveretailinvoicemodel(params).then((res) => {
        if (res.success === true) {
    ...
    handleShopId(value, isInstallShopInfo) {
      ...
      this.formData.new_shop_code = value.code;
    ...
    //Submit按钮
    onSubmit() {
      var test = this.$refs.retailForm.validate();
      ...
      this.paramsSubmit = params;

      this.$toast.loading({ duration: 0, forbidClick: true });
      submitretailinvoicemodel(this.paramsSubmit).then((res) => {
    ...
    addModel() {
      let pushDiv = {
        ...
        new_modelId: "",
        new_model_name: "",
        // item.new_category_id = modelObject.new_category_id;
        // item.new_category_name = modelObject.new_category_name;
        // item.new_sub_category_id = modelObject.new_sub_category_id;
        // item.new_sub_category_name = modelObject.new_sub_category_name;
        // item.model_brand = modelObject.model_brand;
        // item.new_key_model = modelObject.new_key_model;
        // item.new_key_model = modelObject.new_key_model;
        new_key_model: "",

        // item.new_mdm_brand = modelObject.new_mdm_brand;
        new_mdm_brand_name: "",
        // new_unit_price : "",
        new_is_mh_name: "",
        new_model_incentive: "0",
      ...
      getShopSelectData().then((res) => {
        if (res.success === true) {
          ...
          resShop.forEach((element) => {
            ...
            thiselement.code = element.new_shop_code;
      ...
      getmodelbrandbymodel().then((res) => {
        if (res.success === true) {
          ...
          resModel.forEach((element) => {
            ...
            thisElement.Id = element.new_modelId;
            thisElement.text = element.new_model_name;
            this.modelOptions.push(thisElement);
          });
          this.modelFilteredOptions = [];
          this.modelFilteredOptions = this.modelOptions;
        } else {
          this.$toast.fail(res.message);
        }
      });
    ...
    changeModelKey() {
      this.modelFilteredOptions = [];
      for (const i in this.modelOptions) {
        if (
          this.modelOptions[i].text
            .toLowerCase()
            .indexOf(this.searchModelKey.toLowerCase()) >= 0
        ) {
          this.modelFilteredOptions.push(this.modelOptions[i]);
        }
      }
    },
    handleSelectModel(value) {
      ...
      const item = this.addModelList[this.selectModel];
      console.log(item);
      for (const i in this.modelObjects) {
        if (this.modelObjects[i].new_modelId === value.Id) {
          const modelObject = this.modelObjects[i];
          console.log(modelObject.new_key_model, "modelObject.new_key_model");
          item.new_modelId = modelObject.new_modelId;
          item.new_model_name = modelObject.new_model_name;
          // item.new_category_id = modelObject.new_category_id;
          // item.new_category_name = modelObject.new_category_name;
          // item.new_sub_category_id = modelObject.new_sub_category_id;
          // item.new_sub_category_name = modelObject.new_sub_category_name;
          ...
          // item.new_mdm_brand = modelObject.new_mdm_brand;
          item.new_mdm_brand_name = modelObject.new_mdm_brand_name;
          item.new_unit_price = modelObject.new_unit_price;
          item.new_model_incentive = modelObject.new_model_incentive;
          item.new_is_mh_name = modelObject.new_key_model
            ? this.$t("pmt.yes")
            : this.$t("pmt.no");

          item.new_modelid = modelObject.new_modelId;
          item.new_product_id = "";
          item.new_model_type = modelObject.new_model_type;
          // item.new_model_name_name = modelObject.new_model_name + "(" + modelObject.model_brand + ")";
          this.addModelList[this.selectModel] = item;
          break;
        }
      }
      console.log(item);
  ...
  beforeRouteLeave(to, from, next) {
    ...
    if (this.isSaveSuccess || this.type != 3) {
      setTimeout(() => {
        this.$dialog
          .confirm({
            title: this.$t("pmt.test.leaveTitle"),
            message: this.$t("pmt.retail.leavePageMessage"),
          })
          .then(() => {
            next(true);
          })
